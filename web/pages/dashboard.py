...
from battle import get_battle_recommendations
from inventory import get_mega_types, get_pokemon_storage
from pokemon_utils import calculate_cp_from_level, get_form_name_img, get_mega_stats
from constants import mega_pokedex_ids, mega_xy_pokedex_ids, primal_pokedex_ids

from web.pages.add_pokemon import render_add_new_pokemon, render_delete_existing_pokemon, render_update_existing_pokemon, selectbox_pokemon
...
def get_pokemon_card_assets(p):
    ...
    poke_type1_filepath = Path(__file__).parent.parent / "types" / f"POKEMON_TYPE_{poke_type1}.png"
    poke_type2_filepath = (
        Path(__file__).parent.parent / "types" / f"POKEMON_TYPE_{poke_type_2}.png"
        if poke_type_2
        else None
    )
    ...
    poke_img_filepath = Path(__file__).parent.parent / "images" / poke_img_filename
...
def show_pokemon_details(p, poke_img_filepath, IVs, poke_type1_filepath, poke_type2_filepath):
    ...
    with col2:
        ...
        with st.container(horizontal=True):
            fm_type_filename = f"POKEMON_TYPE_{p["fast_type"]}.png"
            fm_type_filepath = Path(__file__).parent.parent / "types" / fm_type_filename
            st.image(fm_type_filepath, width=25)
            st.write(p["fast_name"].replace('_', ' ').title())
        with st.container(horizontal=True):
            c1_type_filename = f"POKEMON_TYPE_{p["c1_type"]}.png"
            c1_type_filepath = Path(__file__).parent.parent / "types" / c1_type_filename
            st.image(c1_type_filepath, width=25)
            st.write(p["c1_name"].replace('_', ' ').title())
        if p["c2_type"]:
            with st.container(horizontal=True):
                c2_type_filename = f"POKEMON_TYPE_{p["c2_type"]}.png"
                c2_type_filepath = Path(__file__).parent.parent / "types" / c2_type_filename
...
def _render_battle_tab(pokemons):
    ...
    pve_filepath, raid__filepath = Path(__file__).parent.parent / "battle_logo" / pve_filename, Path(__file__).parent.parent / "battle_logo" / raid_filename
    if selected_mode == "PvE (Raids & Gyms)":
        with st.container(horizontal_alignment="center"):
            st.image(pve_filepath, width=400)
    elif selected_mode == "PvP (Trainer Battles & GO Battle League)":
        with st.container(horizontal_alignment="center"):
            st.image(raid__filepath, width=110)
    
    dmg_multipy_filename = "DAMAGE_MULTIPLIER.png"
    dmg_multipy_filepath = Path(__file__).parent.parent / "types" / dmg_multipy_filename
    with st.container(horizontal_alignment="center"):
        st.image(dmg_multipy_filepath)
    ...
    if selected_poke:
        ...
        poke_img_filepath = Path(__file__).parent.parent / "images" / poke_img_filename
        poke_type1_filepath = Path(__file__).parent.parent / "types" / f"POKEMON_TYPE_{selected_poke["type_1"]}.png"
        poke_type2_filepath = (
            Path(__file__).parent.parent / "types" / f"POKEMON_TYPE_{selected_poke["type_2"]}.png"
        ...
        if "deck_results" in st.session_state:
            ...
            cols_per_row = 3
            cnt = 1
            for i in range(0, len(result_filtered), cols_per_row):
                cols = st.columns(cols_per_row)
                for col, pokemon in zip(cols, result_filtered[i:i + cols_per_row]):
                    with col:
                        render_deck_card(pokemon, cnt)
                        cnt += 1


def render_deck_card(pokemon, rank):
    p = pokemon["pokemon"]
    IVs = f"{p["atk_iv"]}/{p["def_iv"]}/{p["sta_iv"]}"
    form = get_form_name_img(p["form"])
    ...
    with st.container(border=True):
        ...
        col1, _, col2 = st.columns([1.3, 0.2, 2])
        with col1:
            if _is_mega_or_primal(pokemon):
                st.markdown(
                    f"""
                    <p style="text-align: center; color: #FF0000; font-weight: 500;">
                        CP {p["cp"]} ({IVs})
        ...
        with col2:
            ...
            with st.container(horizontal=True):
                ...
                fm_type_filepath = Path(__file__).parent.parent / "types" / fm_type_filename
                st.image(fm_type_filepath, width=25)
                st.write((p["fast_name"] + _format_effectiveness(pokemon["eff_f"])).replace('_', ' ').title())
                st.write(f"{pokemon["fast_move"]}")
            with st.container(horizontal=True):
                c1_type_filename = f"POKEMON_TYPE_{p["c1_type"]}.png"
                c1_type_filepath = Path(__file__).parent.parent / "types" / c1_type_filename
                st.image(c1_type_filepath, width=25)
                st.write((p["c1_name"] + _format_effectiveness(pokemon["eff_c1"])).replace('_', ' ').title())
                st.write(f"{pokemon["charge_1"]}")
            with st.container(horizontal=True):
                if p["c2_type"]:
                    c2_type_filename = f"POKEMON_TYPE_{p["c2_type"]}.png"
                    c2_type_filepath = Path(__file__).parent.parent / "types" / c2_type_filename
                    st.image(c2_type_filepath, width=25)
                    st.write((p["c2_name"] + _format_effectiveness(pokemon["eff_c2"])).replace('_', ' ').title())
                    st.write(f"{pokemon["charge_2"]}")
